...
class AccountInvoiceLine(orm.Model):
    ...
    def product_id_change(
        ...
        """
        Override the product_id_change.

        Override of product on change in order to set the product invoice
        description in invoice line note. We also set name of product in
        invoice line name.
        """
        # Can you feel the pain?
        base_func = super(AccountInvoiceLine, self).product_id_change

        res = base_func(cr, uid, ids, product, uom_id, qty=qty, name=name,
                        type=type, partner_id=partner_id, context=context,
        ...
        product_note = pobj.read(cr, uid, product,
